...
import { createBuildClient, supabaseConfigured } from '@/lib/supabase/build'
...
async function fetchPageTarget(slug: string): Promise<PageTarget | null> {
  ...
  try {
    ...
    const { data } = await (supabase as any).from('page_targets').select('*').eq('slug', slug).single()
    return (data ?? null) as PageTarget | null
  } catch {
    return null
  }
}

async function fetchRooms(area: string, category: string): Promise<AvailableRoom[]> {
...
export async function generateStaticParams() {
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data } = await (supabase as any).from('page_targets').select('*')
    const targets: PageTarget[] = data ?? []
...
export default async function SanctuaryLandingPage(
  ...
  const target = await fetchPageTarget(`${feature}-${area}`)
  const rooms  = await fetchRooms(areaLabel, category)
  ...
  const jsonLd = target ? buildJsonLd(target, rooms) : []
    ...
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 py-12">
        ...
        <header className="mb-10">
          ...
          {target && (
            <p className="text-lg text-stone-500 max-w-2xl leading-relaxed">{target.meta_desc}</p>
          )}
        </header>
