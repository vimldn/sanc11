...
import { supabaseConfigured } from '@/lib/supabase/build'
      ...
      <div className="flex flex-wrap gap-6 mb-8 pb-6 border-b border-stone-200">
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {AREAS.map((area) => (
              <a key={area} href={`/rooms?area=${area}&category=${selectedCategory}&date=${selectedDate}`}
                className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${selectedArea === area ? 'bg-stone-900 text-white border-stone-900' : 'border-stone-200 text-stone-500 hover:border-stone-400'}`}
              >{area}</a>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {CATEGORIES.map((cat) => (
              <a key={cat.value} href={`/rooms?area=${selectedArea}&category=${cat.value}&date=${selectedDate}`}
                className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${selectedCategory === cat.value ? 'bg-stone-900 text-white border-stone-900' : 'border-stone-200 text-stone-500 hover:border-stone-400'}`}
              >{cat.label}</a>
            ))}
          </div>
        </div>
      ...
      {rooms.length > 0 ? (
        ...
        <div className="border border-stone-200 rounded-xl p-16 text-center">
          <p className="text-stone-400 mb-2">
            {supabaseConfigured() ? 'No rooms available for this filter combination.' : 'Connect Supabase to see available rooms.'}
          </p>
          {supabaseConfigured() && <a href="/rooms" className="text-sm text-gold hover:underline">Clear filters</a>}
        </div>
      )}
    </main>
  )
